...
export const TOOLS: Record<string, ToolDef> = {
  get_state: {
    description:
      "Full table snapshot as you are allowed to see it: both battlefields, graveyards, exiles, command zones, life totals, YOUR hand (Player's hand/library are hidden), zone counts, and the recent event log. Card ids in the snapshot are what every other tool takes. It also answers three questions you would otherwise guess at: `combat` is which combat question is currently open (attackers = who is swinging, blockers = the defender owes a declaration, damage = it is owed and yours to announce, done = damage resolved, null = not in combat), `players.<id>.turnDone` is what each seat has already done this turn (untap, draw, lands played, whether it has acted), and `canMulligan` says whether your opening hand can still be shuffled back.",
    schema: obj({}),
    special: "state",
  },
  draw: {
  ...
  set_phase: { description: "Declare a phase/step change — applies IMMEDIATELY (logged, no stack item). The only turn-structure stack item is the TURN PASS (set_turn); attack/block declarations still go on the stack, and end-of-turn responses happen against the TURN PASS.", schema: obj({ phase: str("one of: untap/upkeep, main 1, combat, main 2, end (aliases like untap, draw, second main, cleanup are folded in, as are a 'beginning of ' prefix and a ' step'/' phase' suffix — 'untap step', 'beginning of combat', 'main phase 1' all land; bare 'main' resolves by whether combat is done)") }, ["phase"]) },
  ...
  block: {
    ...
    schema: obj(
      {
        pairs: arr(obj({ blocker: str("card id"), attacker: str("card id") }, ["blocker", "attacker"]), "blocks"),
        allowTapped: { type: "boolean", description: "an effect lets tapped creatures block (rare) — without this, tapped blockers are refused" },
      },
      ["pairs"]
    ),
  },
