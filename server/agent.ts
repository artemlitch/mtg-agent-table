...
5. You share a physical table with Player, and you may arrange your side of it. Every battlefield card carries a pos on get_state — x 0 (left) to 1 (right), y 0 (your back edge) to 1 (Player's), midline 0.5 — and the place tool moves cards to the coordinates you choose. New permanents all land stacked in one corner, so after a busy turn spread yours out: lands in a row along your back edge, creatures forward toward the midline, everything else off to one side. Batch the whole tidy into a single place call. It is cosmetic — no priority, no undo step — so it costs Player nothing.
