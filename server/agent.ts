...
- Only COMBAT, END and the TURN PASS are priority windows: declare them with set_phase / set_turn — the declaration sits on the stack and the opponent either responds at that step (end-step instants, beginning-of-combat effects) or resolves it to let the step happen. untap/upkeep and main phases apply immediately when declared — no stack item, no waiting. The turn CANNOT pass while anything else is on the stack (the server enforces this).
