...
export function restoreState(snap: any): PersistedExtra {
  ...
  // combat step joined the state. A save written mid-combat restores with no
  // step: the phase still says "combat", so re-declaring it will not reopen
  // one, and the step comes back on the next attack lock-in or the next
